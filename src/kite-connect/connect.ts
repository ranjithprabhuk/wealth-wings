...
const KiteConnect = function (params) {
  ...
  const requestInstance = axios.create({
    ...
    headers: {
      ...
      "x-kite-userid": appConfig.userId,
    },
  ...
  self.getLoginURL = function () {
    return (
      self.default_login_uri + "?api_key=" + self.api_key + "&v=" + kiteVersion
    );
  };
  ...
  self.generateSession = function (request_token, api_secret) {
    return new Promise(function (resolve, reject) {
      const checksum = sha256(
        self.api_key + request_token + api_secret
      ).toString();
      const p = _post(
        "api.token",
        {
  ...
  self.renewAccessToken = function (refresh_token, api_secret) {
    return new Promise(function (resolve, reject) {
      const checksum = sha256(
        self.api_key + refresh_token + api_secret
      ).toString();
...
regular).
   ...
   * @param {string} mode    (optional) Compact mode will only give the total margins
   */
  self.orderBasketMargins = function (
    orders,
  ...
  function request(
    ...
    if (self.access_token) {
      ...
      options["headers"]["authorization"] = "enctoken " + self.access_token;
    }
